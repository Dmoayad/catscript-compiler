...
public class FactorExpression extends Expression {
    ...
    public Object evaluate(CatscriptRuntime runtime) {
        ...
        if (operator.getType().equals(TokenType.STAR)) {
            val = (Integer) lhsValue * (Integer) rhsValue;
        }
        else {
            // division
            val = (Integer) lhsValue / (Integer) rhsValue;
    ...
    public void compile(ByteCodeGenerator code) {
        // compile both hand-sides if it's a multiply operation, add multiply jvm instruction
        // if division use division instruction
